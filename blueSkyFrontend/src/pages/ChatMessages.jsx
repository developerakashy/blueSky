            ...
            <div  className="w-full flex flex-col gap-2 px-2 pb-18 border-blue-400 max-h-[85.5%] overflow-y-auto">
                {messages && messages.map((message, index) =>
                    <div ref={messagesEndRef} key={message?._id} className={`mt-2 max-w-[50%] flex-col ${message?.senderUserId?._id === user?._id ? 'flex items-end self-end' : ''}`}>
                            ...
                            <p className={`mt-[1px] mb-1 text-xs text-gray-500 ${message?.senderUserId?._id === user?._id ? 'text-end mr-1' : 'text-start ml-1'}`}>{message?.createdAt || '26 Jan, 2025, 2:00 PM'}</p>
                    </div>
                )}

            </div>

            <div className="bg-white border-[1px] px-4 py-2 absolute bottom-0 left-0 right-0 flex justify-between items-center">
                <input
                    className="border-[1px] w-[85%] h-10 text-lg px-3 rounded-2xl"
                    type="text"
                    value={msgText}
                    ...
                    ref={inputRef}
                />
                <button onClick={() => handleSendMessage()} className="px-4 py-2 bg-blue-500 text-white rounded-full h-min">send</button>
            </div>
        </div>
    )
}
